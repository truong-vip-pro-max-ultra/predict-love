...
import com.truongjae.predictlove.enumtype.ContentType;
import com.truongjae.predictlove.service.UIDService;
import com.truongjae.predictlove.service.impl.UIDServiceImpl;
...
import static com.truongjae.predictlove.utils.ReflectionUtil.get;
...
//            System.out.println(result);
    ...
    public static <P> P postMethod(String url, String body, Class<P> responseClass, String cookie,boolean clearResp) {
        CloseableHttpResponse response = null;
        try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
            HttpPost post = new HttpPost(url);
            post.addHeader("content-type", "application/x-www-form-urlencoded");
            ...
            if(clearResp){
                ...
                String userId = uidService.getUIDFromCookie(cookie);
                int indexStartCut = userId.length()+4;
                result = result.substring(indexStartCut);
